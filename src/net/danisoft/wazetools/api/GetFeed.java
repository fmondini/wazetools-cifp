...
//       Revised: Jun/2025 The Scheduler part has been REMOVED because Waze can't process it properly
//                         It has been replaced with the new Schedule.getActiveSlotPeriod() flow
//
////////////////////////////////////////////////////////////////////////////////////////////////////
...
public class GetFeed extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			boolean isClosureActive;
			Date datePeriodMin, datePeriodMax, Now = new Date();

			//
			// App Fields
			//

			feed_name = "Generated by " + AppCfg.getAppName() + " " + AppCfg.getAppVersion() + " build " + AppCfg.getAppRelDate();
			...
			for (Group.Data grpData : GRP.getAll("GRP_CtryIso = '" + feedCountry + "'", null)) {
				...
				if (grpData.isActive()) {
					...
					for (Segment.Data segData : SEG.getAll(grpData.getID())) {
						...
						//
						// Check if the closure is active at the time of this call
						//

						isClosureActive = false;

						if (datePeriodMin.getTime() <= Now.getTime() && datePeriodMax.getTime() >= Now.getTime()) {

							// Is between MIN & MAX period

							if (grpData.getSchedule().isEmpty()) {

								// No scheduler data, set as active

								isClosureActive = true;

							} else {

								// Some scheduler data found, check day of week and time slot

								Date[] scheduleSlotPeriod = Schedule.getActiveSlotPeriod(grpData);

								if (scheduleSlotPeriod[0] != null) {

									// At the time of this API call, a closure is active

									isClosureActive = true;

									// Replaces the data from the original period with that from the active slot

									feed_start_time = FmtTool.fmtWazeCifpFeedDateTime(scheduleSlotPeriod[0]);
									feed_end_time = FmtTool.fmtWazeCifpFeedDateTime(scheduleSlotPeriod[1]);
								}
							}
						}
						...
						if (isClosureActive) {
							...
							if (feedFormat.equals(FeedFormat.XML.toString())) {
								...
								// Scheduler
								// ---------
								// In Jun/2025 this part has been REMOVED because Waze can't process it properly
								// It has been replaced with the new Schedule.getActiveSlotPeriod() flow
/*
